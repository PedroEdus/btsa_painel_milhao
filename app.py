...
with tabs[0]:
    banner_premiacao()
    # Query atual: coluna única cupons_casas; formato anterior: sorteio atual.
    _col_casas = next(
        (c for c in ("cupons_casas", "cupons_casas_sorteio_atual") if c in df.columns),
        None,
    )
    _cupons_casa_atual = (
        int(pd.to_numeric(df[_col_casas], errors="coerce").fillna(0).sum())
        if _col_casas else 0
    )
    hero(
        ...
        [
            ...
            f'<i class="fa-solid fa-house"></i> <b>{numero(_cupons_casa_atual)}</b> cupons casas',
